...
const mockFeedbacks: FeedbackView[] = [
  {
    id: "f1",
    profileId: "profile1",
    userName: "Ahmed",
  ...
  },
  {
    ...
    userName: "Sara",
    rating: 4,
    comment: "Loved the pasta, will come again.",
    createdAt: new Date().toISOString(),
  },
];

export default function FeedbackPage() {
  const [feedbacks, setFeedbacks] = useState<FeedbackView[]>(mockFeedbacks);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!userName || rating === 0) return;

    const now = new Date().toISOString();
    const newFeedback: FeedbackView = {
      id: `temp-${Date.now()}`,
      profileId: "profile1", // later: read from searchParams
      userName,
      rating,
